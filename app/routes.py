"""HTTP routes for eligibility-service.

Generic CRUD over the `eligibility` table. Every mutating endpoint publishes a
...
def build_blueprint(svc) -> Blueprint:
    ...
    @bp.get("/<int:record_id>/check")
    @require_auth(scopes=[f"{RESOURCE}.read"])
    def check_eligibility(record_id: int):
        row = db.query_one(f"SELECT * FROM {TABLE} WHERE id = %s", (record_id,))
        if not row:
            return jsonify({"error": "not found"}), 404
        return jsonify({"eligible": True, "policy_id": record_id, "checked_at": row["updated_at"].isoformat()})
